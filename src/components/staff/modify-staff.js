...
export default function ModifyStaff({ staff, modifyStaffAction }) {
    ...
    const { register, handleSubmit, formState: { errors }, reset } = useForm({
        ...
        defaultValues: {
            ...
            dob: dayjs(staff?.dob).format('YYYY-MM-DD'),
            mobile: staff?.mobile,
            gender: staff?.gender
        }
    })
    ...
    const handleCreateStaff = async (values) => {
        let result = await modifyStaffAction(values)
        if(result.code === 200) {
            toast.success(`Staff modified success`)
